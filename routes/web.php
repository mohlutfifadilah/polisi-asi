...
Route::get('/jawab-respon/{token}/{aduan}', function ($token, $aduan) {
    if (!Auth::user()) {
        Alert::warning('Perhatian', 'Anda harus login terlebih dahulu');
        return redirect()->route('login');
    }
    $selectedAduan = Aduan::find($aduan);
    $allAduan = Aduan::where('id', $selectedAduan->id_aduan)
        ->orWhere('id', $aduan)
