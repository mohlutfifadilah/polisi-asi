...
<div class="row row-bordered g-0">
    <div class="col-lg-4 p-4">
        <div class="demo-inline-spacing">
            <form method="get" action="{{ url('/admin/carousel/' . $carousel->id . '/edit') }}">
                <button class="btn btn-warning mr-2" type="submit">
                    <i class="tf-icons mdi mdi-file-edit-outline"></i>
                </button>
            </form>
            <form method="post" action="{{ route('carousel.destroy', $carousel->id) }}"
                id="delete_from_{{ $carousel->id }}">
                @csrf
                @method('DELETE')
                <button class="btn btn-danger _delete_data" type="submit" data-id="{{ $carousel->id }}"
